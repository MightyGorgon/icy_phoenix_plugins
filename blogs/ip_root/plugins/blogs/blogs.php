...
if (!empty($resync))
{
	$class_blogs->resync_articles();
	$message = '<br /><br />' . $lang['BLOGS_RESYNCHED'];
	$message .= '<br /><br />' . sprintf($lang['CLICK_RETURN_BLOGS'], '<a href="' . append_sid(THIS_FILE) . '">', '</a>');
...
$input_allowed = check_auth_level(AUTH_REG) ? true : false;
...
$sql_where_extra = "u.user_id = i.blog_user_id_create AND u2.user_id = i.blog_user_id_update";
// EXTRA DB OVERLAY - END

// SORTING OPTIONS OVERLAY - BEGIN
$sort_order_default = 'blog_time_update';
...
if ($input_allowed)
{
	...
	$breadcrumbs['bottom_right_links'] .= (($breadcrumbs['bottom_right_links'] != '') ? ('&nbsp;' . MENU_SEP_CHAR . '&nbsp;') : '') . '<a href="' . append_sid(THIS_FILE . '?mode=input') . '">' . $lang['BLOG_ADD'] . '</a>';
}
$breadcrumbs['bottom_right_links'] .= (($breadcrumbs['bottom_right_links'] != '') ? ('&nbsp;' . MENU_SEP_CHAR . '&nbsp;') : '') . '<a href="' . append_sid(THIS_FILE) . '">' . $lang['BLOGS_LINK_ALL'] . '</a>';
...
if ($mode == 'save')
{
	...
	$inputs_array['blog_owner'] = empty($inputs_array['blog_owner']) ? ANONYMOUS : $class_form->get_user_id($inputs_array['blog_owner']);
	//$inputs_array['blog_user_id_update'] = ($admin_allowed) ? $inputs_array['blog_user_id_update'] : $user->data['user_id'];
	$inputs_array['blog_user_id_update'] = $user->data['user_id'];
	$inputs_array['blog_time_update'] = $current_time;
	...
	{
		// EXTRA ASSIGNMENTS - BEGIN
		$inputs_array['blog_owner'] = ($inputs_array['blog_owner'] == ANONYMOUS) ? $user->data['user_id'] : $inputs_array['blog_owner'];
		$inputs_array['blog_user_id_create'] = $user->data['user_id'];
		$inputs_array['blog_time_creation'] = $current_time;
		// EXTRA ASSIGNMENTS - END
...
elseif ($mode == 'input')
{
	...
	// EXTRA ASSIGNMENTS - BEGIN
	$items_row['blog_owner'] = $class_form->get_username($items_row['blog_owner']);
	//$table_fields['blog_user_id_update']['default'] = ($admin_allowed && ($action == 'edit')) ? $table_fields['blog_user_id_update']['default'] : $user->data['user_id'];
	//$items_row['blog_user_id_update'] = ($admin_allowed && ($action == 'edit')) ? $items_row['blog_user_id_update'] : $user->data['user_id'];
	// EXTRA ASSIGNMENTS - END
...
elseif ($mode == 'view')
{
	...
	// Page Title - BEGIN
	$item_title = $items_row['blog_title'];
...
{
	$template_to_parse = $class_plugins->get_tpl_file(BLOGS_TPL_PATH, 'blogs_body.tpl');
	$template->assign_vars(array(
	...
	if ($page_items == 0)
	{
		$template->assign_var('NO_BLOGS', true);
	}
	else
	{
		$row_class = '';
		...
		{
			$blog_poster = colorize_username($items_array[$i]['blog_user_id_create'], $items_array[$i]['username'], $items_array[$i]['user_color'], $items_array[$i]['user_active']);

			$blog_last_poster = colorize_username($items_array[$i]['blog_user_id_update'], $items_array[$i]['username2'], $items_array[$i]['user_color2'], $items_array[$i]['user_active2']);

			//$view_link = append_sid(THIS_FILE . '?mode=view&amp;' . $item_id . '=' . $items_array[$i][$item_id]);
			$view_link = append_sid(CMS_PAGE_BLOG . '?' . $class_blogs->blog_id_var . '=' . $items_array[$i][$item_id]);
			$view_img = '<a href="' . $view_link . '"><img src="' . IP_ROOT_PATH . $images['cms_icon_preview'] . '" alt="' . $lang['BLOG_VIEW'] . '" title="' . $lang['BLOG_VIEW'] . '" /></a>';
			...
			$template->assign_block_vars('blogs', array(
				'CLASS' => $row_class,
				//'ROW_NUMBER' => $i + 1,
				'ROW_NUMBER' => $items_array[$i][$item_id],

				'STATUS' => $class_form->get_lang_from_value($items_array[$i]['blog_status'], $table_fields['blog_status']['values']),
				'TITLE' => ((strlen($items_array[$i]['blog_title']) > 55) ? (htmlspecialchars(substr(htmlspecialchars_decode($items_array[$i]['blog_title'], ENT_COMPAT), 0, 52)) . '...') : $items_array[$i]['blog_title']),
				'DATE' => create_date_ip($config['default_dateformat'], $items_array[$i]['blog_time_creation'], $config['board_timezone']),
				'DATE_UPDATE' => create_date_ip($config['default_dateformat'], $items_array[$i]['blog_time_update'], $config['board_timezone']),
				'POSTER' => $blog_poster,
...
$template->assign_vars(array(
	...
	'U_BLOGS_RESYNC' => append_sid(THIS_FILE . '?resync=true'),

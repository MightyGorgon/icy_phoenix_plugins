...
<tr>
	...
	<td class="row2"><input type="text" class="post" size="50" maxlength="120" name="link_logo_src" value="{LINK_LOGO_SRC}">&nbsp;[<a href="javascript: void(0)" onclick="var img_src=document.link_form.link_logo_src.value;if(img_src=='') img_src='images/links/no_logo88a.gif';if(img_src.substr(0, 4) != 'http') img_src='{FULL_SITE_PATH}'+img_src;_preview=window.open(img_src, '_preview', 'toolbar=no,width=200,height=100,top=300,left=300');">{L_PREVIEW}</a>]</td>
</tr>
<tr> 
...
<tr>
	...
	<td class="row2">{LINK_LOGO_IMG}<br /><br /><textarea name="link_desc" cols="30" rows="5" class="post" maxsize="120">{LINK_DESC}</textarea></td>
</tr>

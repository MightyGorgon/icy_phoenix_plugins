...
if ($result)
{
	...
	//$add_user = ($row['user_id'] != '' || $row['user_id'] != ANONYMOUS) ? '<a href="' . append_sid(CMS_PAGE_PROFILE . '?mode=viewprofile&amp;' . POST_USERS_URL . '=' . $row['user_id']) . '">'.$row['username'] . '</a>' : $lang['Guest'];
	$add_user = colorize_username($row['user_id'], $row['username'], $row['user_color'], $row['user_active']);
	$add_time = create_date($config['default_dateformat'], $dl_files['add_time'], $config['board_timezone']);
	$db->sql_freeresult($result);
}
...
if ($dl_files['add_time'] != $dl_files['change_time'])
{
	...
	{
		$row = $db->sql_fetchrow($result);
		//$change_user = ($row['user_id'] != '' || $row['user_id'] != ANONYMOUS) ? '<a href="' . append_sid(CMS_PAGE_PROFILE . '?mode=viewprofile&amp;' . POST_USERS_URL . '=' . $row['user_id']) . '">'.$row['username'] . '</a>' : $lang['Guest'];
		$change_user = colorize_username($row['user_id'], $row['username'], $row['user_color'], $row['user_active']);
		$change_time = create_date($config['default_dateformat'], $dl_files['change_time'], $config['board_timezone']);
		$db->sql_freeresult($result);
	}
}

$last_time_string = ($dl_files['extern']) ? $lang['Dl_last_time_extern'] : $lang['Dl_last_time'];
$last_time = ($dl_files['last_time']) ? sprintf($last_time_string, create_date($config['default_dateformat'], $dl_files['last_time'], $config['board_timezone'])) : $lang['Dl_no_last_time'];
...
if ($dl_config['show_real_filetime'] && !$dl_files['extern'])
{
	...
	if (@file_exists($dl_config['dl_path'].$index[$cat_id]['cat_path'].$dl_files['file_name']))
	{
		$template->assign_block_vars('downloads.real_filetime', array(
			...
			'REAL_FILETIME' => create_date($config['default_dateformat'], @filemtime($dl_config['dl_path'] . $index[$cat_id]['cat_path'] . $file_name), $config['board_timezone'])
			)
		);
	}
}

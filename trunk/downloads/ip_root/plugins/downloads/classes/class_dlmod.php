	...
	*/
	function dlmod()
	{
		...
		*/
		...
		$result = $db->sql_query($sql, 0, 'dl_config_');
		...
		*/
		if ($this->dl_config['use_ext_blacklist'])
		{
			$sql = "SELECT extention FROM " . DL_EXT_BLACKLIST . "
				ORDER BY extention";
			$result = $db->sql_query($sql, 0, 'dl_ext_bl_');
		...
		*/
		$auto_overall_traffic_month = create_date('Ym', $this->dl_config['traffic_retime'], $config['board_timezone']);
		...
		*/
		$auto_click_reset_month = create_date('Ym', $this->dl_config['dl_click_reset_time'], $config['board_timezone']);
		$current_traffic_month = create_date('Ym', time(), $config['board_timezone']);
	...
	{
		global $config, $path_dl_array, $bbcode;
	...
	function read_dl_dirs($download_dir, $path = '')
	{
		...
		{
			$exclude_array = array('.', '..', '.htaccess', 'index.html', 'index.htm');
			$current_file = $dl_dir . $path . '/' . $file;
			if (!in_array($file, $exclude_array))
			{
				if(is_dir($current_file))
				{
					...
					$folders .= ('/' . $cur != $path . '/' . $file) ? '<option value="' . $current_file . '/">' . $lang['Dl_move'] . ' &raquo; ' . $temp_dir . '/</option>' : '';
	...
	function read_dl_files($download_dir, $path = '')
	{
		...
		while (false !== ($file = @readdir($dir)))
		{
			$exclude_array = array('.', '..', '.htaccess', 'index.html', 'index.htm');
			$current_file = $dl_dir . $path . '/' . $file;
			if (!in_array($file, $exclude_array) && !is_dir($current_file))
			{
	...
	function read_dl_sizes($download_dir, $path = '')
	{
		...
		while (false !== ($file = @readdir($dir)))
		{
			...
			if (!in_array($file, $exclude_array) && !is_dir($current_file))
			{
				$file_size += sprintf("%u", @filesize($current_file));

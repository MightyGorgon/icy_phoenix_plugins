...
$template->set_filenames(array('activity_daily_section' => ACTIVITY_MOD_PATH . 'activity_daily_body.tpl'));
...
for ($x = 0; $x < $games_c; $x++)
{
	...
	for ($j = 0; $j <= $rating_count; $j++)
	{
		if ( $games_data[$x]['game_id'] == $rating_info[$j]['game_id'] )
		{
			...
			{
				$rating_submit = '<a href="' . append_sid('javascript:Trophy_Popup(\'activity_popup.' . PHP_EXT . '?mode=rate&game=' . $games_data[$x]['game_id'] . '\', \'New_Window\', \'450\', \'300\', \'yes\')') . '">' . $lang['game_rating_submit'] . '</a>';
			}
		}
	}
	...
	$row_class = (!($i % 2)) ? 'row1' : 'row2';
		...
		if ( ($challenge == '1') && ($t_player_id != ANONYMOUS) && ($userdata['user_id'] != ANONYMOUS) )
			$challenge_link = '<br />'. $lang['seperator'] .'&nbsp;<a href="' . append_sid('javascript:Trophy_Popup(\'activity_popup.' . PHP_EXT . '?mode=challenge&g= '. $game_id . '&u=' . $t_player_id . '\', \'New_Window\', \'400\', \'200\', \'yes\')') . '" class="nav">' . $lang['challenge_link_key'] . '</a>';
		...
		if ($games_data[$x]['jackpot'])
			$show_jack = ($game_type != 2) ? '<br />' . $lang['seperator'] . '&nbsp;' . str_replace('%X%', intval($games_data[$x]['jackpot']), $lang['jackpot_text']) : '';
		$games_cost_line = $show_fees . $show_ge . $show_jack;
		...
		$template->assign_block_vars('daily_game', array(
			...
			'MOUSE' => (($game_mouse) ? '<img src="' . ACTIVITY_MOD_PATH . 'images/mouseamod.png" alt="' . $lang['game_mouse'] . '" title="' . $lang['game_mouse'] . '" />' : ''),
			'KEYBOARD' => (($game_keyboard) ? '<img src="' . ACTIVITY_MOD_PATH . 'images/keyboard.png" alt="'. $lang['game_keyboard'] .'" title="'. $lang['game_keyboard'] . '" />' : ''),
			...
			'RUNNER_IMG' => ($game_type != 2) ? '<img src="' . ACTIVITY_MOD_PATH . 'trophy.gif" alt="" />' : '',
			...
			'LINKS' => GameArrayLink($game_id, $game_parent, $game_popup, $win_width, $win_height, 1, $games_data[$x]['game_links']) . (($game_category) ? '<br /><span class="gensmall"><b></b>&nbsp;' . $game_category . '</span>': '')
			)
		);
	break;
		}

...
function AMP_Gender($gender)
	...
	if ($gender == 1)
		return ($lang['amp_char_cp_gender_m'] .'  <img src="' . IP_ROOT_PATH . ACTIVITY_MOD_PATH . 'amp_characters/images/male.gif">');

	if ($gender == 2)
		return ($lang['amp_char_cp_gender_f'] .'  <img src="' . IP_ROOT_PATH . ACTIVITY_MOD_PATH . 'amp_characters/images/female.gif">');
	}
...
function AMP_Profile_Char($user, $char_profile)
	{
	global $userdata, $db, $lang, $board_config, $bbcode;
...
$profile = '';
	for ($a = 0; $a < count($info); $a++)
		...
		$char_intrests = ($board_config['allow_smilies']) ? $bbcode->parse(stripslashes(trim(strip_tags($info[$a]['ina_char_intrests'])))) : stripslashes(trim(strip_tags($info[$a]['ina_char_intrests'])));
		...
		$profile .= '<table width="100%" cellspacing="0" cellpadding="0" align="center" valign="top" class="forumline">';
		$profile .= '	<tr>';

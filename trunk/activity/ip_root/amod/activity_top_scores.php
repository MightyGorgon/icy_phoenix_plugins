...
if (!$search)
{
	$template->set_filenames(array('body' => ACTIVITY_MOD_PATH . 'activity_top_scores_body.tpl') );
	...
	for ($a = $start; $a < ($start + $end); $a++)
	{
		for ($b = 0; $b < count($u_data); $b++)
		{
			if ($t_data[$a]['player'] == $u_data[$b]['user_id'])
			{
				...
				$pro = append_sid(PROFILE_MG . '?mode=viewprofile&amp;' . POST_USERS_URL . '=' . $u_data[$b]['user_id']);
				$profile = '<a href="' . $pro . '"><img src="' . $images['icon_profile'] . '" alt="' . $lang['Profile'] . '" title="' . $lang['Profile'] . '" border="0" /></a>';

				$game_image = '';
				for ($c = 0; $c < count($g_data); $c++)
				{
					if ($g_data[$c]['game_name'] == $game_name)
					{
						$game_image = '<center>' . $g_data[$c]['proper_name'] . '</center><br />' . GameArrayLink($g_data[$c]['game_id'], $g_data[$c]['game_parent'], $g_data[$c]['game_popup'], $g_data[$c]['win_width'], $g_data[$c]['win_height'], '3%SEP%'. CheckGameImages($game_name, $g_data[$c]['proper_name']), '');
						break;
					}
				}
				...
				$template->assign_block_vars('top_scores_rows', array(
					...
					'USER_SEARCH' => '<a href="activity.' . PHP_EXT . '?page=trophy_search&amp;user=' . $fix_user_n . '&amp;sid=' . $userdata['session_id'] . '" ' . $user_color . '>' . $user_n . '</a>',
...
$pagination = generate_pagination('activity.' . PHP_EXT . '?page=trophy', count($t_data), $board_config['games_per_page'], $start). ' ';

$template->assign_vars(array(
	'PAGINATION' => $pagination,
	'PAGE_NUMBER'  => sprintf($lang['Page_of'], floor(($start / $board_config['games_per_page']) + 1), ceil(count($t_data) / $board_config['games_per_page']))
	)
);

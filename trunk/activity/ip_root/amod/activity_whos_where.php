...
$template->set_filenames(array('body' => ACTIVITY_MOD_PATH . 'activity_whos_where_body.tpl'));
...
while($row = $db -> sql_fetchrow($r))
{
	...
	//$mailto = ($board_config['board_email_form']) ? append_sid(PROFILE_MG . '?mode=email&amp;' . POST_USERS_URL . '=' . $row1['user_id']) : 'mailto:' . $row1['user_email'];
	$mailto = ($board_config['board_email_form'] && !empty($row1['user_viewemail'])) ? append_sid(PROFILE_MG . '?mode=email&amp;' . POST_USERS_URL . '=' . $row1['user_id']) : '';
	...
	$pro = append_sid(PROFILE_MG . '?mode=viewprofile&amp;' . POST_USERS_URL . '=' . $row1[user_id]);

...
/* Start Restriction Checks */
	...
	if(($board_config['ina_disable_gamble_page']) && ($userdata['user_level'] != ADMIN)) message_die(GENERAL_ERROR, $lang['disabled_page_error'], $lang['ban_error']);
...
/* End Restriction Checks */
	...
	if($board_config['use_rewards_mod'])
		...
		$points_name = $board_config['ina_cash_name'];
			}
		}

	$template->set_filenames(array('body' => ACTIVITY_MOD_PATH . 'activity_gambling_body.tpl'));
	...
	if($amount > $board_config['ina_max_gamble']) message_die(GENERAL_ERROR, $lang['gambling_max_exceeded_error'], $lang['gambling_error']);
	...
	if(($board_config['use_point_system']) && ($board_config['use_rewards_mod']) && ($free_fee == "2"))
		{
	...
	if(($board_config['use_cash_system'] || $board_config['use_allowance_system']) && ($board_config['use_rewards_mod']) && ($free_fee == "2"))
		{
	...
	if ($_GET['mode'] == '')
	{
		...
		$template->assign_block_vars('bet_selection', array(
			...
			'L_MAX_BET_DESC' => str_replace("%a%", $board_config['ina_max_gamble'] ." ". $points_name, $lang['gambling_max_bet']),

...
$template->set_filenames(array('activity_newest_section' => ACTIVITY_MOD_PATH . 'activity_newest_body.tpl'));
...
for ($i = 0; $i < $game_count; $i++)
{
	...
	for ($j = 0; $j <= count($download_data); $j++)
	{
		...
		{
			$download_link = '<br />&nbsp;<a href="downloads.php?mode=download&amp;cid=910&amp;lid=' . $download_data[$j]['lid'] . '&amp;sid=' . $userdata['session_id'] . '" class="nav"><font color="#339933">Download This</font></a><br />';
			break;
		}
	}
	...
	$template->assign_block_vars('newest', array(
		...
		'MOUSE' => (($game_mouse) ? '<img src="' . ACTIVITY_MOD_PATH . 'images/mouseamod.png" alt="'. $lang['game_mouse'] .'" title="'. $lang['game_mouse'] .'" /><br />' : ''),
		'KEYBOARD' => (($game_keyboard) ? '<img src="' . ACTIVITY_MOD_PATH . 'images/keyboard.png" alt="' . $lang['game_keyboard'] . '" title="' . $lang['game_keyboard'] . '" /><br />' : ''),
		...
		'LINKS' => GameArrayLink($game_id, $game_parent, $game_popup, $win_width, $win_height, 1, $game_rows[$i]['game_links']) . (($game_category) ? '<br /><span class="gensmall"><b></b>&nbsp;'. $game_category .'</span>': '')
		)
	);
}

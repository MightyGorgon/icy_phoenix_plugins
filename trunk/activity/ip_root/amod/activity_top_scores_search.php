...
if(($board_config['ina_disable_trophy_page']) && ($userdata['user_level'] != ADMIN)) message_die(GENERAL_ERROR, $lang['disabled_page_error'], $lang['ban_error']);
...
if ($search)
{
	...
	while($row = $db->sql_fetchrow($r1))
	{
		...
		$mailto = ( $board_config['board_email_form'] ) ? append_sid(PROFILE_MG . '?mode=email&amp;' . POST_USERS_URL . '=' . $row['user_id']) : 'mailto:' . $row['user_email'];

...
$template->set_filenames(array('activity_info_section' => ACTIVITY_MOD_PATH . 'activity_info_body.tpl'));
...
$top_player2 = str_replace('%t%', '<a href="' . append_sid('activity.' . PHP_EXT . '?page=trophy_search&amp;user=' . $usersname) . '" class="nav">' . $userstrophies . '</a>', $top_player1);
...
if ($userdata['user_trophies'] > 0)
	$users_trophies = '<b>' . $lang['seperator_2'] . '</b>&nbsp;' . str_replace('%t%', '<a href="' . append_sid('activity.' . PHP_EXT . '?page=trophy_search&amp;user=' . $userdata['username']) . '" class="nav">' . $userdata['user_trophies'] . '</a>', $lang_to_use);
...
if ($board_config['use_cash_system'] || $board_config['use_allowance_system'])
	$cash_fix = $board_config['default_reward_dbfield'];
if ( ($board_config['use_rewards_mod']) && ($board_config['use_point_system']) )
	$onhand = '<b>' . $lang['seperator_2'] . '</b>&nbsp;' . str_replace('%p%', number_format($userdata['user_points']) .' '. $board_config['points_name'], $lang['info_box_user_points']);
if ( ($board_config['use_rewards_mod']) && ($board_config['use_cash_system'] ||
	$board_config['use_allowance_system']) ) $onhand = '<b>' . $lang['seperator_2'] . '</b>&nbsp;' . str_replace('%p%', number_format($userdata[$cash_fix]) . ' ' . $board_config['ina_cash_name'], $lang['info_box_user_points']);
...
$template->assign_block_vars('info_box', array(
	...
	'USERNAME'	 => $users_link,
	'TOTAL_GAMES_LINK' => '<b>' . $lang['seperator_2'] . '</b>&nbsp;' . str_replace('%t%', '<a href="'. append_sid('activity.' . PHP_EXT . '?page=high_scores&amp;mode=highscore&amp;player_search=' . $userdata['username']) . '" class="nav">' . $total_scores . '</a>', $game_lang),

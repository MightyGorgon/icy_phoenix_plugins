	...
	if(($board_config['ina_use_rating_reward']) && ($board_config['ina_rating_reward'] > 0))
	{
		if($board_config['use_rewards_mod'])
		{
			...
			if($board_config['use_cash_system'] || $board_config['use_allowance_system'])
			{
				...
				add_reward($userdata['user_id'], $board_config['ina_rating_reward']);
				$msg = str_replace("%P%", $board_config['ina_rating_reward'] .'&nbsp;'. $points_name, $lang['rating_payout_message']);
			}
		}
	}
...
$template->set_filenames(array('body' => ACTIVITY_MOD_PATH . 'activity_popup_body.tpl') );
...
if ($mode == 'chat')
{
	...
	if ($action == 'history')
	{
		...
		$start = str_replace('%S%', '<tr><td align="left" class="row2" colspan="2"><span class="genmed">', $chat_session);
	...
	$template->assign_block_vars('comments.main', array(
		...
		for ($a = 0; $a < $total_comments; $a++)
		{
			...
			$template->assign_block_vars('comments.comment', array(
				'TROPHY_HOLDER' => '<a href="'. append_sid(PROFILE_MG . '?mode=viewprofile&amp;' . POST_USERS_URL . '=' . $comment_left_id) . '">' . $comment_left_name . '</a>',

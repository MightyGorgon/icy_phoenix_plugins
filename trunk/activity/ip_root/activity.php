...
if (isset($_GET['page']))
{
	...
	if ($what_page == 'hof')
	{
		$page_to_load = (ACTIVITY_ROOT_PATH . 'activity_hof.' . PHP_EXT);
	}
	if ($what_page == 'trophy')
	{
	...
	{
		$page_to_load = (ACTIVITY_ROOT_PATH . 'activity_top_scores_search.' . PHP_EXT);
	}
	if ($what_page == 'challenges')
	{
		$page_to_load = (ACTIVITY_ROOT_PATH . 'challenges.' . PHP_EXT);
	}
	...
	if ($what_page == 'top')
	{
		...
		$page_to_load = (ACTIVITY_ROOT_PATH . 'activity_top_five.' . PHP_EXT);
	}
	if ($what_page == 'services')
	{
		$page_name = $lang['services_page_title'];
		$page_to_load = (ACTIVITY_ROOT_PATH . 'activity_services.' . PHP_EXT);
	}
	if ($what_page == 'high_scores')
	{
		$page_to_load = (ACTIVITY_ROOT_PATH . 'activity_highscores.' . PHP_EXT);
	}
	if ($what_page == 'search')
	{
		$page_name = $lang['search_title'];
		$page_to_load = (ACTIVITY_ROOT_PATH . 'activity_search.' . PHP_EXT);
	}
	if ($what_page == 'whos_where')
	{
		$page_name = $lang['Who_is_Online'];
		$page_to_load = (ACTIVITY_ROOT_PATH . 'activity_whos_where.' . PHP_EXT);
	}
	if ($what_page == 'settings')
	{
		$page_to_load = (ACTIVITY_ROOT_PATH . 'activity_settings.' . PHP_EXT);
	}


	#==== Load The Page Title
	$meta_content['page_title'] = $lang['page_title_' . $what_page];
...
$finish = ($user_use_games_count > 1) ? $user_use_games_count : $config['games_per_page'];
...
if (($mode == 'category_play') && (!$_GET['cat']))
{
	...
	$new_length = time() - 86400 * $config['ina_new_game_limit'];
	...
	$pop_limit = $config['ina_pop_game_limit'];
	...
	$q = "SELECT COUNT(game_id) AS total_popular, cat_id
				FROM " . iNA_GAMES . "
				WHERE played > '". $config['ina_pop_game_limit'] ."'
				GROUP BY cat_id";
	$r = $db->sql_query($q);
...
else
{
	...
	$game_cost = ($config['use_point_system']) ? $config['points_name'] : $lang['game_cost'];
	...
	else
	{
		...
		$gamelib_link = ($config['use_gamelib'] == 1) ? '<div align="center"><span class="copyright">' . $lang['game_lib_link'] . '</span></div>' : '';
		...
		while ($row = $db->sql_fetchrow($r1))
		{
			...
			$template->assign_block_vars('drop', array(
				'D_SELECT_1' => htmlspecialchars($game_i2),
				'D_SELECT_2' => htmlspecialchars($game_n2)
				)
			);
		}
		...
		for ($i = 0; $i < $game_count; $i++)
		{
			...
			for ($j = 0; $j <= sizeof($rating_info); $j++)
			{
			...
			{
				$comments_link = 'javascript:popup_open(\'' . append_sid('activity_popup.' . PHP_EXT . '?mode=comments&amp;game=' . urlencode($game_name)) . '\',\'New_Window\',\'550\',\'300\',\'yes\')';
			}
		...
		if (($mode == 'category_play') && ($_GET['cat']))
		{
			...
			{
				$header_logo_image = $config['ina_use_logo'];
			}
		}
		else
		{
		...
		$template->assign_vars(array(
			...
			'S_ACTION' => append_sid('activity.' . PHP_EXT),
...
if ($user_use_info)
{
	include_once(ACTIVITY_ROOT_PATH . 'activity_info.' . PHP_EXT);
}

if (($config['ina_use_daily_game']) && ($user_use_daily))
{
	include_once(ACTIVITY_ROOT_PATH . 'activity_daily.' . PHP_EXT);
}

if ($_GET['mode'] != 'category_play')
{
	if (($config['ina_use_newest'] == 1) && ($user_use_newest))
	{
		include_once(ACTIVITY_ROOT_PATH . 'activity_newest.' . PHP_EXT);
	}
}

if (($config['ina_use_online'] == '1') && ($user_use_online))
{
	include_once(ACTIVITY_ROOT_PATH . 'activity_online.' . PHP_EXT);
}

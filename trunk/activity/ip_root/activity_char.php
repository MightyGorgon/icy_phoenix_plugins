...
/* End Restriction Checks */
	...
	if ($mode == 'create_char' || $mode == 'edit_char' || $mode == 'del_char' || $mode == 'profile_char' || !$mode)
		$template->set_filenames(array('body' => ACTIVITY_MOD_PATH . 'activity_char_body.tpl') );

	if ($mode == 'char_shop')
		$template->set_filenames(array('body' => ACTIVITY_MOD_PATH . 'activity_char_shop_body.tpl') );
	...
	for ($x = 0; $x < count($saying_color_split); $x++)
		{
	...
	for ($x = 0; $x < count($title_color_split); $x++)
		{
	...
	for ($x = 0; $x < count($name_color_split); $x++)
		{
	...
	if ($mode == 'create_char' || !$mode)
		...
		if (!$userdata['ina_char_name'])
			...
			$m_dir = IP_ROOT_PATH . ACTIVITY_MOD_PATH . 'amp_characters/male/';
			...
			$f_dir = IP_ROOT_PATH . ACTIVITY_MOD_PATH . 'amp_characters/female/';
	...
	$error = '';
		...
		$male = IP_ROOT_PATH . ACTIVITY_MOD_PATH . 'amp_characters/male/' . $char_img_m;
		$female = IP_ROOT_PATH . ACTIVITY_MOD_PATH . 'amp_characters/female/' . $char_img_f;

...
if( !empty($setmodules) )
{
	...
	$module['Amod+ Admin']['.:Configuration:.'] = $file;
	$module['Amod+ Admin']['Add_Game'] = append_sid('admin_activity.' . PHP_EXT . '?mode=add_game');
	$module['Amod+ Admin']['Edit_Games'] = append_sid('admin_activity.' . PHP_EXT . '?mode=edit_games');
	return;
}
...
include_once(IP_ROOT_PATH . ACTIVITY_MOD_PATH . 'includes/functions_amod_plus.' . PHP_EXT);
	...
	if ($mode == 'add_game')
	{
		$template->set_filenames(array('body' => ADM_TPL . 'game_add_body.tpl') );
	...
	if ($mode == 'edit_games')
		...
		if (!$action)
			{
		$template->set_filenames(array('body' => ADM_TPL . 'activity_admin_main.tpl') );
		...
		if ($action == 'view')
			{
		$template->set_filenames(array('body' => ADM_TPL . 'activity_admin_edit.tpl') );
		$cat = ($_GET['cat']) ? $_GET['cat'] : $_GET['cat'];
			...
			$template->assign_block_vars('cat_choice', array());
				...
				$game_image = '<img src="'. IP_ROOT_PATH . $board_config['ina_default_g_path'] .'/'. $games_data[$a]['game_name'] .'/'. $games_data[$a]['game_name'] .'.gif" border="0">';
					if (!$game_image)
	...
	if (!$result = $db->sql_query($sql))
		...
		{
		if (($board_config['default_reward_dbfield'] != 'user_money') && ($board_config['use_allowance_system']))
			{
		...
		if (($board_config['default_reward_dbfield'] != $board_config['default_cash']) && ($board_config['use_cash_system']))
			{
		$sql = "UPDATE " . CONFIG_TABLE . "
	...
	$template->set_filenames(array('body' => ADM_TPL . 'activity_config_body.tpl'));
	...
	$template->assign_vars(array(
		...
		"VERSION" => $version,
		"DASH" => $lang['game_dash'],
		"DEFAULT_CASH" => $board_config['default_cash'],

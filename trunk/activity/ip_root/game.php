...
if ($userdata['user_level'] == ADMIN)
{
	$proper_name = '<a href="#" onclick="popup_open(\'' . ACTIVITY_ADM_PATH . '/admin_activity.' . PHP_EXT . '?mode=edit_games&amp;action=edit&amp;game=' . $game_info['game_id'] . '&amp;sid=' . $userdata['session_id'] . '\', \'New_Window\', \'550\', \'300\', \'yes\'); return false;">' . $game_info['proper_name'] . '</a>';
}
else
{
	$proper_name = $game_info['proper_name'];
...
elseif (($game_flash) && ($_GET['parent']))
{
	...
	$template->assign_vars(array(
		...
		'T_LINK_1' => $t_holder_name . '\'s <a href="' . append_sid('activity.' . PHP_EXT . '?page=trophy_search&amp;user=' . urlencode($t_holder_name) . '&amp;sid=' . $userdata['session_id']) . '">'. $lang['game_profile'] .'</a>',
		'T_IMAGE' => ACTIVITY_IMAGES_PATH . 'trophy.gif',
		'R_TITLE' => $lang['top_ten'],

		'U_PLAY_POPUP' => '<a href="#" onclick="popup_open(\'activity.' . PHP_EXT . '?mode=game&amp;id=' . $game_info['game_id'] . '&amp;sid=' . $userdata['session_id'] . '\', \'New_Window\', \'550\', \'300\', \'yes\'); return false;">' . $lang['new_window'] . '</a>',

...
$meta_content['page_title'] = $lang['Page_title'];
...
if (isset ($_GET['start']))
{
	...
	$template->set_filenames(array('body' => KB_ADM_TPL_PATH . 'admin_message_body.tpl'));
	...
	if (($start + $num_rows) != $total_num_rows) {
		...
		$redirect_url = KB_ADM_PATH . $form_action;
	...
	$template->set_filenames(array('body' => KB_ADM_TPL_PATH . 'kb_rebuild_search_progress.tpl'));
}
else
{
	$template->assign_vars (array (
	...
	$template->set_filenames (array ('body' => KB_ADM_TPL_PATH . 'kb_rebuild_search.tpl'));
}

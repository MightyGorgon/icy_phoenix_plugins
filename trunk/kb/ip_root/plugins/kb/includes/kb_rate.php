...
if ($rate == 'dorate')
{
	...
	if (!$reader_mode)
	{
		$message = $conf . '<br /><br />' . sprintf($lang['Click_return_rate'], '<a href="' . append_sid(this_kb_mxurl('mode=article&amp;k=' . $article_id)) . '">', '</a>') . '<br /><br />' . sprintf($lang['Click_return_forum'], '<a href="' . append_sid(CMS_PAGE_FORUM . '?page=' . $page_id . '&amp;mode=cat&amp;cat=' . $category_id) . '">', '</a>');
	}
	else
	{

...
if (sizeof($kb_row) > 0)
{
	...
	$date = create_date_ip($config['default_dateformat'], $kb_row['article_date'], $config['board_timezone']);
...
// Parse message
$article = censor_text($article);
$article_title = censor_text($article_title);
$bbcode->allow_html = true;
$bbcode->allow_smilies = ($config['allow_smilies'] && $smilies_on && !$lofi) ? true : false;
...
$meta_content['page_title'] = $article_title;
...
if ($kb_config['formatting_fixup'])
{
	...
	$kb_art_description = censor_text($kb_art_description);
...
{
	$message = $lang['Article_not_exsist'] . '<br /><br />' . sprintf($lang['Click_return_kb'], '<a href="' . append_sid(this_kb_mxurl()) . '">', '</a>') . '<br /><br />' . sprintf($lang['Click_return_index'], '<a href="' . append_sid(IP_ROOT_PATH . CMS_PAGE_FORUM) . '">', '</a>');
...
else
{
	...
	if ($kb_is_auth['auth_comment'] && $kb_config['use_comments'])
	{
		...
		$temp_url = append_sid(IP_ROOT_PATH . CMS_PAGE_VIEWTOPIC . '?' . POST_TOPIC_URL . '=' . $topic['topic_id']);
	...
	if (sizeof($art_pages) > 1)
	{
		...
		while ($i < sizeof($art_pages))
		{
			...
			$article_toc = censor_text($article_toc);
